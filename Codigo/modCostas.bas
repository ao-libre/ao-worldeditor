...
Public Sub PutCoast()
    ...
    Static antAux(4) As Byte 'Para el movimiento del grh (como las costas de pasto estan compuestas por 2 grhs c/u en forma de : 1 0 1 0 donde 1 es la costa y el 0 es el grh en negro (que no nos interesa)
        ...
        If Y > YMinMapSize And Y < YMaxMapSize Then
            ...
            For X = XMinMapSize To XMaxMapSize
                ...
                If X > XMinMapSize And X < XMaxMapSize Then
                    ...
                    If HayAgua2(X - 1, Y) And HayAgua2(X - 1, Y - 1) And HayAgua2(X, Y - 1) And HayAgua2(X + 1, Y - 1) And HayAgua2(X + 1, Y) And HayAgua2(X + 1, Y + 1) And HayAgua2(X, Y + 1) And HayAgua2(X - 1, Y + 1) Then
...
Function HayAgua2(ByVal X As Integer, ByVal Y As Integer) As Boolean
    HayAgua2 = ((MapData(X, Y).Graphic(1).GrhIndex >= 1505 And MapData(X, Y).Graphic(1).GrhIndex <= 1520) Or (MapData(X, Y).Graphic(1).GrhIndex >= 5665 And MapData(X, Y).Graphic(1).GrhIndex <= 5680) Or (MapData(X, Y).Graphic(1).GrhIndex >= 13547 And MapData(X, Y).Graphic(1).GrhIndex <= 13562))
